<template>
  <nav v-if="hasPagination" class="p-2 border-t border-gray-100 dark:border-slate-800">
    ...
    <div v-if="hasData" class="flex-1 flex justify-between" :class="{ 'sm:hidden': hasLinks }">
      ...
      <div class=" flex flex-row">
        ...
        <span @click="gototo">
          <button ref="button" type="button" title="go to top"
        ...
        <span @click="gotobottom">
          <button ref="button" type="button" title="go to bottom"
            class="border border-gray-200 dark:border-slate-700 text-opacity rounded-md shadow-sm p-1 ml-2 inline-flex justify-center text-sm font-medium hover:border-gray-600 hover:dark:border-slate-100 border-gray-300">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 " viewBox="0 0 24 24" fill="currentColor">
              <path
        ...
        <span v-if="props.selectedRows.length > 0">&nbsp;{{ props.selectedRows.length }} row(s) Selected</span>
      </div>
    ...
    <div v-if="hasData && hasLinks" class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
      <div class="flex flex-row space-x-4 items-center flex-grow">
        ...
        <p class="hidden lg:block text-sm text-gray-700 dark:text-gray-200 flex-grow">
          ...
          <span @click="gototo">
            <button ref="button" type="button" title="go to top"
              ...
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-7 " viewBox="0 0 24 24" fill="currentColor">
                <path
                  d="M13,18V10L16.5,13.5L17.92,12.08L12,6.16L6.08,12.08L7.5,13.5L11,10V18H13M12,2A10,10 0 0,1 22,12A10,10 0 0,1 12,22A10,10 0 0,1 2,12A10,10 0 0,1 12,2Z" />
              </svg>
            </button>
            ...
            <slot name="link">
              <component :is="link.url ? 'a' : 'div'" v-if="!isNaN(link.label) || link.label === '...'
                ...
                :class="{
                  ...
                  'bg-gray-300 dark:!bg-slate-500': link.active,
...
const gototo = () => {
  document.getElementById('datatable').rows[0].scrollIntoView({ behavior: 'smooth' });
}
const gotobottom = () => {
