<template>
  <fieldset ref="tableFieldset" :key="`table-${name}`" :dusk="`table-${name}`" class="min-w-0"
      ...
      <div class="flex flex-row flex-wrap sm:flex-nowrap justify-end px-0 py-1 pr-2 ">
        ...
        <slot name="tableAddSearchRow" :has-search-inputs="queryBuilderProps.hasSearchInputs"
          ...
          <TableAddSearchRow v-if="queryBuilderProps.hasSearchInputs && !props.hideSearchColumn"
            class="order-2 sm:order-4 mr-2 ml-2 sm:mr-2 pt-1" :search-inputs="queryBuilderProps.searchInputsWithoutGlobal"
        ...
        <div class="order-4 mx-2  sm:order-5 pt-1" v-if="bulkDelete">
          <div class="relative">
            <button type="button" title="Delete Selected" :disabled="!selectedRows.length"
        ...
        <div class="order-4 mx-2  sm:order-5 pt-1"
          v-if="props.resourceNeo.servExp && props.resourceNeo.bulkActions && props.resourceNeo.bulkActions.csvExport">
          <div class="relative">
            <a title="All Data Export"
              class="w-full border border-gray-200 dark:border-slate-700 text-opacity rounded-md shadow-sm px-4 py-2 inline-flex justify-center text-sm font-medium hover:border-gray-600 hover:dark:border-slate-100 border-gray-300"
              :href="route(props.resourceNeo.resourceName + '.export')" target="_blank">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 " viewBox="0 0 20 20" fill="currentColor">
                <path fill="currentColor"
    ...
    <div v-if="props.resourceNeo.cellDetail" class="px-2  flex flex-row">
      ...
      <input type="text"
        ...
        v-model="detailCellRef" v-show="(true)" id="dcellselectid" @keydown.enter.prevent="goto">
      ...
      <div class="ml-1" v-show="true">
        <button type="button" @click="hx" :disabled="props?.resourceNeo?.cellGoToHistoryDisabled"
          class="w-full border border-gray-200 dark:border-slate-700 rounded-md shadow-sm px-2 py-2  inline-flex justify-center text-sm font-medium hover:border-gray-600 hover:dark:border-slate-100 border-gray-300 bg-gray-200 dark:bg-slate-700 text-gray-400"
          :title="props?.resourceNeo?.cellGoToHistoryDisabled ? 'Not yet functional' : 'Hx'">
          <span>Hx</span>
        </button>
      </div>
          ...
          <table id="datatable" cllass="border-t border-gray-500 mx-2 rounded" v-columns-resizable>
            ...
            <tbody>
              <slot name="body" :show="show">
                <tr v-for="(  item, key  ) in   resourceData  " :key="`tr-${item.id}`" class="border-b border-t" :class="{
                  ...
                  ">
                  <td v-if="!props.resourceNeo.DisCheckboxes" :data-id="item.id"><input type="checkbox" ref="rowCheckboxes" :value="item.id"
                      :checked="selectedRows.includes(item.id)" @click.prevent="selectItem(item, key, $event)" /></td>
                  <td v-for="column   in   queryBuilderProps.columns  " :key="`td-${column.key}-${item.id}`"
                    v-show="show(column.key)" class="text-sm p-0 visited:outline outline-white outline-1 !bg-opacity-75"
                    :dataid="item.id" :dataval="item[column.key]" :datakey="column.key" @dblclick.prevent="showpopper()"
                    ...
                    :colKey="column.extra.colKey" :id="`${column.extra.colKey}${item.id}`" :dispval="column.extra.dispval"
  ...
  <CardBoxModal class="Filter-and-Search" v-model="isModalSearchPopupActive" buttonLabel="Filter" title="Filter and Search" button="sucess"
    has-cancel :fullWidth="(true)">
    <div class="grid grid-cols-1 lg:grid-cols-4 xl:grid-cols-5 gap-1 red-color -mb-6">
      <TableWideSearch :filters="queryBuilderProps.searchInputsWithoutGlobal" :columns="queryBuilderProps.columns"
        v-model="searchFields" :resourceName="props.resourceNeo.resourceName" @formSubmit="doPopupSearch" />
    </div>
  ...
  <CardBoxModal :infoText="infotext" v-model="isEditPopupActive" buttonLabel="Update" :title="editPopupTitile"
    button="info" has-cancel @confirm="popupEditFunction" @cancel="gotowithotmessage">
    <input v-if="editPopupFieldType == 'input' && !editPopupFieldNumberOnly" v-model="editPopupModel"
    ...
    <input v-if="editPopupFieldType == 'input' && editPopupFieldNumberOnly" v-model="editPopupModel" type="number"
      @keydown.enter.prevent="isEditPopupActive = false; popupEditFunction()" ref="poperinput"
      class="px-3 py-1 max-w-full focus:ring focus:outline-none border-gray-700 rounded w-full dark:placeholder-gray-400 h-10 border bg-white dark:bg-slate-800" />
    ...
    <VueDatePicker input-class-name="text-gray-500 dark:text-gray-100 shadow-sm text-sm bg-gray-100 dark:bg-slate-600"
      :month-change-on-scroll="false" :range="false" :enable-time-picker="false" arrow-navigation format="yyyy-MM-dd"
      auto-apply v-if="editPopupFieldType === 'datePicker'" :model-value="editPopupModel"
      @update:model-value="setFilterValue" uid="poperinput" teleport-center>
    </VueDatePicker>

    <div
      v-if="editPopupFreeTextFieldType && (editPopupFieldType == 'select' || editPopupFieldType == 'input' || editPopupFieldType == 'checkbox' || editPopupFieldType === 'datePicker')">
      ...
      <br><br>
      <textarea v-model="editFreeTextPopupModel" @keydown="inputHandlerTextarea"
      ...
      <input v-if="editPopupFreeTextFieldType == 'input'" v-model="editFreeTextPopupModel"
        @keydown.enter.prevent="isEditPopupActive = false; popupEditFunction()"
    ...
    <div v-if="infotext" class="text-sm text-gray-500 -mb-2 text-wrap text-balance text-justify -mb-2 infotext-mb-ff">{{ infotext }}</div>
    <div
  ...
  <CardBoxModal v-model="isModelViewableActive" buttonLabel="Copy to Clipboard" title="View Data" has-cancel
    @confirm="copyVieabletoClipboard">
    <textarea v-model="editPopupModel"
  ...
  <div style="width:0px;height:0px;"><input id="dummyinput" style="width:0px;height:0px;border:0; padding:0"></div>
</template>
...
const emit = defineEmits(["selectedRows", 'selectedCell', 'saveExportActivityLog']);
...
const saveExportActivityLog = ref(null);
...
const gotocell = (el) => {
  if (el) {
    if (el.style && el.style.display == 'none') {
      useToast().error('Cell Reference found but hidden!! Expand Col to see.');
    }
...
function showpopper() {
  ...
  if (el != null) {
    ...
    if (!queryBuilderData.value.columns[columnIndex].extra.editable) {
      ...
      else if (queryBuilderData.value.columns[columnIndex].extra.dblClEventTrOn) {
        var doubleClickEvent = document.createEvent('MouseEvents');
        doubleClickEvent.initEvent('dblclick', true, true);
        document.getElementById(queryBuilderData.value.columns[columnIndex].extra.dblClEventTrOn + id).dispatchEvent(doubleClickEvent);
      }
      else if (queryBuilderData.value.columns[columnIndex].extra.clEventTrOn) {
        var doubleClickEvent = document.createEvent('MouseEvents');
        doubleClickEvent.initEvent('click', true, true);
        document.getElementById(queryBuilderData.value.columns[columnIndex].extra.clEventTrOn + id).dispatchEvent(doubleClickEvent);
      }
      else {
    ...
    editPopupFieldNumberOnly.value = queryBuilderData.value.columns[columnIndex].extra.onlyNumber ? true : false;
    ...
    if (typeof queryBuilderData.value.columns[columnIndex].extra.type == 'undefined') {
      ...
      editPopupModel.value = { 'id': el.getAttribute("dataval"), 'label': el.innerText };
      var indexedArray = [];
      ...
      let temparray = el.getAttribute("dataval") ? el.getAttribute("dataval").split(/\r?\n/) : [];
    ...
    if (queryBuilderData.value.columns[columnIndex].extra.type == 'datePicker') {
      setTimeout(function () { document.getElementById('dp-input-poperinput').focus() }, 100);
    }
...
const rowCheckboxes = ref({});
const lastSelectedRow = ref(null);

const selectItem = (item, key, event) => {
    setTimeout(function () {
        if (event.shiftKey && lastSelectedRow.value !== key) {
            let selectedkeys = [];
            let allKeys = Object.keys(rowCheckboxes.value);

            if (lastSelectedRow.value < key) {
                selectedkeys = allKeys.slice(lastSelectedRow.value + 1, key + 1);
            } else {
                selectedkeys = allKeys.slice(key, lastSelectedRow.value);
            }

            selectedkeys.forEach((value) => {
                let itemId = parseInt(rowCheckboxes.value[value].value);

                if (!selectedRows.value.includes(itemId)) {
                    selectedRows.value.push(itemId)
                } else {
                    selectedRows.value.splice(selectedRows.value.indexOf(itemId), 1)
                }
            });
        } else {
...
defineExpose({
  resetSelect, showHideAll, focustoDisplayCell
});
