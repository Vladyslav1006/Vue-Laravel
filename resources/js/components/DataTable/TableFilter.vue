<template>
  <ButtonWithDropdown placement="bottom-end" dusk="filters-dropdown" :active="hasEnabledFilters" title="Filters">
    <template #button>
    ...
    <div role="menu" aria-orientation="horizontal" aria-labelledby="filter-menu" class="min-w-max">
      <div v-for="(filter, key) in filters" :key="key" class="flex flex-row py-1">
        <div class="w-2/5 text-xs uppercase tracking-wide  text-gray-900 dark:text-gray-100  text-opacity py-3 pr-3">
          {{ filter.label }}
        </div>
        <div class="w-3/5 p-0">
          ...
          <VueDatePicker
            ...
            v-if="filter.type === 'datePicker'" :model-value="filter.value"
            @update:model-value="setFilterValue(filter.key, $event)" :name="filter.key">
          </VueDatePicker>
        </div>
      </div>
    </div>
